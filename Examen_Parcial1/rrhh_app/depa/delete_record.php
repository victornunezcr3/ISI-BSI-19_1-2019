			...
			<div id="content"><!-- Start of content for the delete page -->
				...
				<?php 
					...
					if ($_POST['sure'] == 'Yes') { // Delete the record.
						// Make the query:
							$q = "DELETE FROM departamentos WHERE id_departamento=$id LIMIT 1";		
					...
					// Print a message:
							...
							echo '<h3>The departament has NOT been deleted.</h3>';	
						}
					} else { // Show the form.
						// Retrieve the departament's information:
						$q = "SELECT CONCAT(id_departamento, ' ', nombre_dep) FROM departamentos WHERE id_departamento=$id";
						$result = @mysqli_query ($dbcon , $q);
						if (mysqli_num_rows($result) == 1) { // Valid deparatment ID, show the form.
							// Get the departament's information:
							$row = mysqli_fetch_array ($result, MYSQLI_NUM);
							// Display the record being deleted:
							echo "<h3>Are you sure you want to permanently delete the departament id -->$row[0]?</h3>";
							// Create the form:
